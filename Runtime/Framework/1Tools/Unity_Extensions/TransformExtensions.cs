...
namespace GG.Extensions
{
	public static class TransformExtensions
    {
        ...
        public static T AddComponent<T>(this Component cp) where T : Component
        {
           return cp.gameObject.AddComponent<T>();
        }
        
        public static string GetPath(this Component component, Transform topLevel) 
        {
            if (component.transform == topLevel)
            {
                return component.transform.name;
            }
            
            return component.transform.parent.GetPath(topLevel) + "/" + component.transform.name;
        }
        ...
        public static float CaculateMovementTime(this Transform t, Vector3 location, float speed)
        {
            float distance = Vector3.Distance(location, t.position);
            return distance / speed;
        }
        
        public static void LookAtLocked(this Transform transform, Vector3 worldPosition, Vector3 worldUp, bool lockX, bool lockY, bool LockZ)
        {
            Vector3 ogRot = transform.localEulerAngles;
            Vector3 newRot = Quaternion.LookRotation(worldPosition, worldUp).eulerAngles;
