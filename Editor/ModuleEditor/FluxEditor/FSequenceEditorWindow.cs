...
namespace FluxEditor
{
	...
	public class FSequenceEditorWindow : EditorWindow
	{
		...
		void OnSelectionChange()
		{
            ...
            PrefabAssetType prefabType = PrefabUtility.GetPrefabAssetType(Selection.activeGameObject);

            FSequence sequence = Selection.activeGameObject == null || prefabType == PrefabAssetType.Regular ||  prefabType == PrefabAssetType.Variant ? null : Selection.activeGameObject.GetComponent<FSequence>();
		...
		{	
			...
			else if( Event.current.isKey )
			{
				...
				{
					bool isOnTimeLineY = Event.current.mousePosition.y > FSequenceWindowHeader.HEIGHT + FSequenceWindowHeader.ONE_LINE_HEIGHT;

                    if ( Event.current.type == EventType.KeyUp && isOnTimeLineY)
					{
