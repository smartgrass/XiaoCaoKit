		...
		private bool SyncWithAnimationWindow { 
			...
			set {
				...
				else
				{
					...
					if( AnimationWindowProxy.InAnimationMode() )
						AnimationWindowProxy.StopAnimationMode();
				}
		...
		private void OnUpdate()
		{
			...
			if( numEvts > 0 )
			{
				...
				if( animEvt.ControlsAnimation )
				{
					...
					if( animWindowFrame >= 0 && animWindowFrame != normCurrentFrame && !SequenceEditor.IsPlaying ) // only allow to control from AnimationWindow when not playing
					{
