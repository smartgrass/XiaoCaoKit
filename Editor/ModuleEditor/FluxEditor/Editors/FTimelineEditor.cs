...
namespace FluxEditor
{
	public class FTimelineEditor : FEditorList<FTrackEditor>
	{
	    ...
	    private static List<KeyValuePair<Type, FEventAttribute>> _validTypeList = null;
        ...
        private void ShowAddTrackMenu()
		{
		    ...
		    if (_validTypeList == null)
		    {
		        _validTypeList = new List<KeyValuePair<Type, FEventAttribute>>();

		        Assembly[] allAssemblies = AppDomain.CurrentDomain.GetAssemblies();

		        Type typeOfEvent = typeof(FEvent);

                for (int i = 0, numAssemblies = allAssemblies.Length; i != numAssemblies; ++i)
		        {
		            Assembly assembly = allAssemblies[i];

		            Type[] assemblyTypes = assembly.GetTypes();

		            for (int j = 0, numTypes = assemblyTypes.Length; j != numTypes; ++j)
		            {
		                Type type = assemblyTypes[j];

		                if (!typeOfEvent.IsAssignableFrom(type)) { continue; }
		                ...
		                _validTypeList.Add(new KeyValuePair<Type, FEventAttribute>(type, (FEventAttribute)attributes[0]));
                    }
		        }

		        _validTypeList.Sort(delegate (KeyValuePair<Type, FEventAttribute> x, KeyValuePair<Type, FEventAttribute> y)
		        {
		            return x.Value.menu.CompareTo(y.Value.menu);
		        });
            }
			
			foreach( KeyValuePair<Type, FEventAttribute> kvp in _validTypeList )
			{
		...
		void AddTrackMenu( object param )
		{
			...
			FrameRange frameRange = SequenceEditor.ViewRange;
			frameRange.Length = Timeline.Sequence.Length / 4;
            Debug.Log($"FLog add Track ");
			FTrack track = (FTrack)typeof(FTimeline).GetMethod("Add", new Type[]{typeof(FrameRange)}).MakeGenericMethod( kvp.Key ).Invoke( Timeline, new object[]{ frameRange } );
